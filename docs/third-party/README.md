...
| [google-ai-edge/mediapipe](https://github.com/google-ai-edge/mediapipe) | Face landmarks, for placing an object on a face | Apache-2.0 for the code and the model bundle; no commercial restriction | Optional Python extra; the feature it improves runs without it |
| [opencv/opencv_zoo](https://github.com/opencv/opencv_zoo) (YuNet) | Face detection for the editing suite | MIT; no commercial restriction | Fetched at setup, pinned by SHA-256; falls back to OpenCV's bundled cascade |

The local `/tools` page and `npm run tools --` expose catalog status. Only pinned-source installation and local activation state are automated. Provider credentials and platform account authorization remain explicit follow-up steps. Social publishing is not part of this catalog: it uses a hosted engine (Bundle.social, Zernio, or Buffer) configured from `/publish`, and each social platform is still connected inside that engine's own dashboard. Meta Ads Kit is the exception for runtime packaging: TrendRelay installs its exact pinned Social Flow CLI dependency inside the tool directory, while still leaving authentication to the account holder.
