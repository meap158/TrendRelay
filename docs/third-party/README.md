...
The local `/tools` page and `npm run tools --` expose catalog status. Only pinned-source installation and local activation state are automated. Provider credentials and account authentication remain explicit follow-up steps. Meta Ads Kit is the exception for runtime packaging: TrendRelay installs its exact pinned Social Flow CLI dependency inside the tool directory, while still leaving authentication to the account holder.
