...
Because of that the enumerator can drive the operator's **own** Chrome instead of launching one. Set `DOUYIN_CDP_URL` to a running Chrome's debugging endpoint and it attaches there — opening its own tab, leaving other tabs alone, not closing the browser on exit — and harvests from that trusted, warmed session, which is the one Douyin will actually paginate. To expose the endpoint, start Chrome with `--remote-debugging-port=9222` (a normal, already-warmed profile is the point; close other Chrome windows first so the profile is not locked), then run with `DOUYIN_CDP_URL=http://127.0.0.1:9222`. Without the variable it falls back to the persistent-profile launch, which still clears the service-exception block and returns the first page.
