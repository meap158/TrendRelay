...
- Douyin downloads enter the library as `reference-only`. Governed OpenMontage artifacts inherit only the explicitly recorded `owned`, `licensed`, or `public-domain` classification; all other cases remain `unknown`.
- Rights are one of `owned`, `licensed`, `public-domain`, `reference-only`, `unknown`, or `prohibited`. Only the first three are publishable. Rights changes require an owner or approver, governed assurance, explicit confirmation, a written basis, and an audit event.
